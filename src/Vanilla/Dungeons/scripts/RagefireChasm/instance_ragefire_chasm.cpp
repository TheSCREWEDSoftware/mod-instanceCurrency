...
public:
    ...
    {
        instance_ragefire_chasm_InstanceMapScript(Map* map) : InstanceScript(map) {Initialize();}
        ...
        bool SetBossState(uint32 type, EncounterState state) override
        {
            ...
            if (state == DONE)
            {
                switch (type)
                {
                case DATA_OGGLEFLINT:
                case DATA_TARAGAMAN_THE_HUNGERER:
                case DATA_BAZZALAN:
                case DATA_JERGOSH_THE_INVOKER:
                default:
                    break;
                }
            }
